...
if __name__ == "__main__":
    ...
    for row in range(sheet_1.nrows):
        if sheet_1.cell(row,log_speed).value < 10 and sheet_1.cell(row,log_record_type).value == "GpsRecord":
            point_get = {"lat":sheet_1.cell(row,log_latitude).value,"long":sheet_1.cell(row,log_longitude).value}
#            time_data = float(sheet_1.cell(row,log_time).value)
            json_point = {
                    "no":row,
#                   "time":sheet_1.cell(row,log_time).value,
